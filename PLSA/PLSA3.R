...
DTM<- DocumentTermMatrix(
     data_corpus,
     control = list(    
         PlainTextDocument=T,#纯文本
         stripWhitespace=T,  #去空白
         removeNumbers = T,  #去数字   
         removePunctuation = T,#去标点 
         tolower=T,          #小写 
         stopwords=stopwords("english"),#停用词          
         stemDocument =T
      ))
...
for(k in 1:max.step){
    ...
    if(k>5&sum((elpre-el)^2)<1e-10){
        break;
    }
    elpre=el
}
el
z1=order(ep[1,],decreasing=TRUE)[1:20]
ep[1,z1];z1
z2=order(ep[2,],decreasing=TRUE)[1:20]
ep[2,z2];z2
z3=order(ep[3,],decreasing=TRUE)[1:20]
ep[3,z3];z3
z4=order(ep[4,],decreasing=TRUE)[1:20]
ep[4,z4];z4
